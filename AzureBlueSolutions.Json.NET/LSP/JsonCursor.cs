...
/// <summary>
/// Text edit abstraction (zero-based LSP-friendly coordinates).
/// </summary>
public sealed record TextEdit(TextRange Range, string NewText);

/// <summary>
/// Kinds of cursor nodes.
/// </summary>
...
/// <summary>
/// Strongly-typed cursor over a JSON path with name/value ranges and editing helpers.
/// </summary>
public sealed class JsonCursor
{
    ...
    private JsonCursor(string path, JsonCursorKind kind, TextRange? name, TextRange? value, JToken token, string? parentPath)
    {
        Path = path;
        Kind = kind;
        NameRange = name;
    ...
    /// <summary>
    /// Creates a cursor from a parsed result and JSON path.
    /// </summary>
    ...
    /// <summary>Converts the current token to T using Newtonsoft.</summary>
    public T? As<T>() => Token.ToObject<T>();

    /// <summary>
    /// Replaces the value at the cursor using its ValueRange. Caller applies edit to the document text.
    /// </summary>
    ...
    /// <summary>
    /// Inserts a property into an object cursor as '"name": value' before the closing '}'.
    /// Returns a text edit that should be applied to the document text.
    /// </summary>
    public TextEdit? InsertProperty(string documentText, string name, JToken value, bool addCommaIfNeeded = true, string indent = "  ")
    {
    ...
    /// <summary>
    /// Inserts an array item at the end (or at index if provided). Returns a text edit.
    /// </summary>
    ...
    private static TextEdit? InsertPropertyIntoObject(string text, JObject obj, string name, JToken value, bool addComma, string indent)
    {
    ...
    private static TextEdit? InsertArrayItemCore(string text, JArray array, JToken value, int? index, bool addComma)
    {
        ...
        {
            if (TryGetArrayElementSpan(text, start, end, index.Value, out int _, out var elemEnd))
            {
                insertPoint = elemEnd;
    ...
    private static bool TryGetArrayElementSpan(string text, int arrayStart, int arrayEnd, int index, out int elemStart, out int elemEnd)
    {
        elemStart = -1; elemEnd = -1;
        var i = arrayStart + 1;
        var depth = 0;
        var inString = false;
        var currentIndex = 0;
        ...
        while (i < arrayEnd)
        {
            ...
            if (!inString)
            {
                if (ch == '"') { inString = true; if (currentIndex == index && candidateStart < 0) candidateStart = i; i++; continue; }
                if (ch is '[' or '{') { depth++; if (currentIndex == index && candidateStart < 0) candidateStart = i; i++; continue; }
    ...
    {
        var off = 0; var curLine = 0; var i = 0;
    ...
    {
        var l = 0; for (int i = 0; i < offset; i++) if (text[i] == '\n') l++; return l;
    }

    private static int ColOf(string text, int offset)
    {
        var i = offset - 1; while (i >= 0 && text[i] != '\n') i--; return offset - (i + 1);
    }

    /// <summary>
    /// Returns the indentation (spaces/tabs) of the line that contains the given offset.
    /// </summary>
    [RequiresUnreferencedCode("This method may use reflection or serialization that is not compatible with trimming. See https://learn.microsoft.com/dotnet/core/deploying/trimming/trim-warnings/il2026")]
